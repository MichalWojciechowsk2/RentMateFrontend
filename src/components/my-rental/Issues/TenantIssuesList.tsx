...
interface TenantIssuesListProps {
  refresh?: number;
}

const TenantIssuesList: React.FC<TenantIssuesListProps> = ({ refresh }) => {
  ...
  if (loading)
    return (
      <div className="text-gray-500 text-sm mt-4">Ładowanie historii...</div>
    );
            ...
            <div>
              ...
              {issue.status === 2 && issue.resolvedAt && (
                <div className="text-xs text-green-600 mt-1">
                  Naprawiono: {new Date(issue.resolvedAt).toLocaleDateString()}
                </div>
              )}
            </div>
